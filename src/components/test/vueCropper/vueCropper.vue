...
<template>
  ...
  >
    <div class="cropper-box" v-if="imgs">
      <div
        ...
        :style="{
          ...
          transform: `scale(${scale}, ${scale}) translate3d(${x / scale}px, ${
            y / scale
          }px, 0) rotateZ(${rotate * 90}deg)`,
        }"
      >
    ...
    <div
      class="crop-safe-model cropper-move"
      v-show="cropping"
      :style="{
        width: cropW + 'px',
        height: cropH + 'px',
      ...
      @touchstart="cropMove"
    >
      <div class="crop-safe-left-top">
        <div class="crop-border"></div>
      </div>
      <div class="crop-safe-left">
        <!-- <div class="crop-border"></div> -->
      </div>
      <div class="crop-safe-top-right">
        <div class="crop-border"></div>
      </div>
      <div class="crop-safe-top">
        <!-- <div class="crop-border"></div> -->
      </div>
      <div class="crop-safe-right-bottom">
        <div class="crop-border"></div>
      </div>
      <div class="crop-safe-right">
        <!-- <div class="crop-border"></div> -->
      </div>
      <div class="crop-safe-bottom-left">
        <div class="crop-border"></div>
      </div>
      <div class="crop-safe-bottom">
        <!-- <div class="crop-border"></div> -->
      </div>
    </div>
    <div class="crop-scale-change">
      <div class="crop-scale-up" @click="changeScale(-1)">
        <i class="el-icon-minus"></i>
      </div>
      <div class="crop-scale-down" @click="changeScale(1)">
    ...
    getCropChecked(cb, w = 0, h = 0) {
      ...
      img.onload = () => {
        if (this.cropW !== 0) {
          ...
          let width = (this.cropW - w * 2) * dpr;
          let height = (this.cropH - h * 2) * dpr;
    ...
    changeCrop(w, h) {
      ...
      if (this.centerBox) {
        ...
        if (h > axis.y2 - axis.y1) {
          // 高度超标
          scaleY = h / (axis.y2 - axis.y1);
          // h = axis.y2 - axis.y1;
          // w = (h / this.fixedNumber[1]) * this.fixedNumber[0];
        }
      }
      if (w < 237) {
        scaleX = Math.ceil(237 / w);
      }
      if (h < 375) {
        scaleY = Math.ceil(375 / h);
      }
      console.log(scaleX, scaleY, "11111");
      if (scaleX > scaleY) {
        this.scale = scaleX;
      } else {
        this.scale = scaleY;
...
.crop-safe-model {
  div {
    position: absolute;
    background: rgba($color: #fafbff, $alpha: 0.2);
  }
  .crop-border {
    width: 28px;
    height: 28px;
    border: 3px solid #0286df;
    position: absolute;
    background: rgba($color: #fafbff, $alpha: 0);
  }
}
